...
(defn setup-hover-shapes [page-id move-stream selected objects transform selected ctrl? hover hover-ids]
  (let [query-point
        ...
        roots (mf/use-memo
               ...
               (fn []
                 (let [shapes (->> selected (mapv #(get objects %)))]
                   (cp/remove-children objects selected))))]
...
(defn setup-viewport-modifiers [modifiers-stream modifiers selected objects render-ref]
  (mf/use-effect
   (mf/deps modifiers)
  ...
  (hooks/use-stream
   ...
   (fn [modifiers]
     (let [shapes (->> selected (mapv #(get objects %)))
           roots (cp/remove-children objects shapes)
